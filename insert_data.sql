...
VALUES ('P', 'Premium', 'Access to all conference events plus attend the workshop of your choice.',
        TRUE),
       ('S', 'Standard',
        'Access to all conference keynotes,sessions,community open spaces and the exhibition hall',
        FALSE),
       ('C', 'Community', 'Access to keynotes,community open spaces and the exhibition hall',
        FALSE);

INSERT INTO pricing_categories (pricing_category_code, pricing_category_name, pricing_start_date,
                                pricing_end_date)
...
select setval('attendees_attendee_id_seq',
              COALESCE((select max(attendee_id) + 1 from attendees), 1));
select setval('attendee_tickets_attendee_ticket_id_seq',
              COALESCE((select max(attendee_ticket_id) + 1 from attendee_tickets), 1));
select setval('discount_codes_discount_code_id_seq',
              COALESCE((select max(discount_code_id) + 1 from discount_codes), 1));
select setval('session_schedule_schedule_id_seq',
              COALESCE((select max(schedule_id) + 1 from session_schedule), 1));
...
select setval('ticket_prices_ticket_price_id_seq',
              COALESCE((select max(ticket_price_id) + 1 from ticket_prices), 1));
select setval('time_slots_time_slot_id_seq',
              COALESCE((select max(time_slot_id) + 1 from time_slots), 1));
select setval('workshops_workshop_id_seq',
              COALESCE((select max(workshop_id) + 1 from workshops), 1));